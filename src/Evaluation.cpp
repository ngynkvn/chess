//
// Created by Nicholas Miller on 4/17/18.
//

#include "Board.cpp"
#include "Move.cpp"
#include "Evaluation.h"

// pass parameters minimax(current Board, 4, -10000, 10000, true);
eval_pair mini_max(Board game_state, int depth, int alpha, int beta, bool is_max_player) {

    // is the depth zero
    if (depth == 0) return eval_pair(game_state, evaluate(game_state));

    // are there no more possible game states
    vector<Board> children_states = get_states(game_state);
    if (children_states.size() == 0) return eval_pair(game_state, evaluate(game_state));

    // maximize and minimize the possible moves
    if (is_max_player) // if it the players turn
    {
        eval_pair v = eval_pair(NULL, alpha);
        for (Board b : children_states) {
            eval_pair v_prime = mini_max(b, depth-1, v, beta, !max);
            if (v_prime.second > v.second) v = v_prime;
            if (v > beta) return eval_pair(b, beta);
        }
        return v;
    }
    else // if it's the opponents turn
    {
        eval_pair v = eval_pair(NULL, beta);
        for (Board b : children_states) {
            eval_pair v_prime = mini_max(b, depth-1, alpha, v, !max);
            if (v_prime.second < v.second) v = v_prime;
            if (v.second < alpha) return eval_pair(b, alpha);
        }
        return v;
    }
}

int evaluate(Board game_state) {
    int evaluation = 0;
    Piece** curr_board = game_state.getBoard();
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            evaluation += get_piece_value(curr_board[i][j], i, j);
        }
    }
    return evaluation;
}

int get_piece_value(Piece p, int x, int y) {
    switch (p.getPieceCode()) {
        // empty piece evaluation
        case epcEmpty:
            return 0;

            // white piece evaluation
        case epcWpawn:
            return white_pawn_table[x][y];
        case epcWknight:
            return white_knight_table[x][y];
        case epcWbishop:
            return white_bishop_table[x][y];
        case epcWrook:
            return white_rook_table[x][y];
        case epcWqueen:
            return white_queen_table[x][y];
        case epcWking:
            return white_king_table[x][y];

            // black piece evaluation
        case epcBpawn:
            return black_pawn_table[x][y];
        case epcBknight:
            return black_knight_table[x][y];
        case epcBbishop:
            return black_bishop_table[x][y];
        case epcBrook:
            return black_rook_table[x][y];
        case epcBqueen:
            return black_queen_table[x][y];
        case epcBking:
            return black_king_table[x][y];
    }
}


